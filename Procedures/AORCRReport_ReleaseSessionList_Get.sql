...
/****** Object:  StoredProcedure [dbo].[AORCRReport_ReleaseSessionList_Get]    Script Date: 6/18/2018 4:27:03 PM ******/
DROP PROCEDURE [dbo].[AORCRReport_ReleaseSessionList_Get]
GO
...
CREATE PROCEDURE [dbo].[AORCRReport_ReleaseSessionList_Get]
...
BEGIN
	---------------------------------------------------------------------------------------------------------
	---------------- CR REPORT USES THIS PROCEDURE IN THE SESSION SUB REPORT DATABASE LINKS -----------------
	---------------------------------------------------------------------------------------------------------
	...
	--Add empty header row, used to make sure header is always created and row for cloning to create new records
		SELECT '' as X
		  ,-1 as ReleaseSessionID
		  ,'' as ReleaseSession
		  ,'' as SessionNarrative
		  ,99999 as StartDate
		  ,'' as EndDate
		  ,SUM(rs.Duration) as Duration
		  ,SUM(m.[Total Tasks]) as [Total Tasks]
		  ,SUM(m.[Percent Open]) as [Percent Open]
		  ,SUM(m.[Percent Closed]) as [Percent Closed]
		  ,SUM(m.[Total Closed]) as [Total Closed]
		  ,SUM(m.[Carry-In (Closed)]) as [Carry-In (Closed)]
		  ,SUM(m.[New (Closed)]) as [New (Closed)]
		  ,SUM(m.[Carry-Out (Closed)]) as [Carry-Out (Closed)]
		  ,SUM(m.[Total Open]) as [Total Open]
		  ,SUM(m.[Carry-In (Open)]) as [Carry-In (Open)]
		  ,SUM(m.[New (Open)]) as [New (Open)]
		  ,SUM(m.[Carry-Out (Open)]) as [Carry-Out (Open)]
		  ,SUM(m.[Dev (Resources)]) as [Dev (Resources)]
		  ,SUM(m.[Biz (Resources)]) as [Biz (Resources)]
		  ,SUM(m.[Total Resources]) as [Total Resources]
		  ,0 as Sort 
		  ,0 as ARCHIVE
		  ,'' as Z
		  FROM ReleaseSession rs
		...
		SELECT '' as X
		  ,0 as ReleaseSessionID
		  ,'' as ReleaseSession
		  ,'' as SessionNarrative
		  ,99999 as StartDate
		  ,'' as EndDate
		  ,SUM(rs.Duration)/COUNT(isnull(rs.Duration, 1)) as Duration
		  ,SUM(m.[Total Tasks])/COUNT(isnull(m.[Total Tasks], 1)) as [Total Tasks]
		  ,SUM(m.[Percent Open])/COUNT(isnull(m.[Percent Open], 1)) as [Percent Open]
		  ,SUM(m.[Percent Closed])/COUNT(isnull(m.[Percent Closed], 1)) as [Percent Closed]
		  ,SUM(m.[Total Closed])/COUNT(isnull(m.[Total Closed], 1)) as [Total Closed]
		  ,SUM(m.[Carry-In (Closed)])/COUNT(isnull(m.[Carry-In (Closed)], 1)) as [Carry-In (Closed)]
		  ,SUM(m.[New (Closed)])/COUNT(isnull(m.[New (Closed)], 1)) as [New (Closed)]
		  ,SUM(m.[Carry-Out (Closed)])/COUNT(isnull(m.[Carry-Out (Closed)], 1)) as [Carry-Out (Closed)]
		  ,SUM(m.[Total Open])/COUNT(isnull(m.[Total Open], 1)) as [Total Open]
		  ,SUM(m.[Carry-In (Open)])/COUNT(isnull(m.[Carry-In (Open)], 1)) as [Carry-In (Open)]
		  ,SUM(m.[New (Open)])/COUNT(isnull(m.[New (Open)], 1)) as [New (Open)]
		  ,SUM(m.[Carry-Out (Open)])/COUNT(isnull(m.[Carry-Out (Open)], 1)) as [Carry-Out (Open)]
		  ,SUM(m.[Dev (Resources)])/COUNT(isnull(m.[Dev (Resources)], 1)) as [Dev (Resources)]
		  ,SUM(m.[Biz (Resources)])/COUNT(isnull(m.[Biz (Resources)], 1)) as [Biz (Resources)]
		  ,SUM(m.[Total Resources])/COUNT(isnull(m.[Total Resources], 1)) as [Total Resources]
		  ,0 as Sort 
		  ,0 as ARCHIVE
		  ,'' as Z
		  FROM ReleaseSession rs
