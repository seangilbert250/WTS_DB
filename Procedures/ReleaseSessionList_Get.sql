...
DROP PROCEDURE [dbo].[ReleaseSessionList_Get]
GO
...
CREATE PROCEDURE [dbo].[ReleaseSessionList_Get]
...
BEGIN
	...
	--Add empty header row, used to make sure header is always created and row for cloning to create new records
		SELECT '' as X
		  ...
		  ,'' as ReleaseSession
		  ,'' as SessionNarrative
		  ,0 as PrimarySessionManagerID
		  ,'' as [Primary Session Manager]
		  ,0 as SecondarySessionManagerID
		  ,'' as [Secondary Session Manager]
		  ,99999 as StartDate
		  ,'' as EndDate
		  ,SUM(rs.Duration)/COUNT(isnull(rs.Duration, 1)) as Duration
		  ,SUM(m.[Total Tasks])/COUNT(isnull(m.[Total Tasks], 1)) as [Total Tasks]
		  ,round(SUM(m.[Percent Open])/COUNT(isnull(m.[Percent Open], 1)), 0) as [Percent Open]
		  ,round(SUM(m.[Percent Closed])/COUNT(isnull(m.[Percent Closed], 1)), 0) as [Percent Closed]
		...
		SELECT '' as X
		  ...
		  ,rs.ReleaseSession
		  ,rs.SessionNarrative
		  ,rs.PrimarySessionManagerID
		  ,pRes.USERNAME as [Primary Session Manager]
		  ,rs.SecondarySessionManagerID
		  ,sRes.USERNAME as [Secondary Session Manager]
		  ,rs.StartDate
		...
		left join #Metrics m
		on rs.ReleaseSessionID = m.ReleaseSessionID
		left join WTS_RESOURCE pRes on rs.PrimarySessionManagerID = pRes.WTS_RESOURCEID
		left join WTS_RESOURCE sRes on rs.PrimarySessionManagerID = sRes.WTS_RESOURCEID
		WHERE rs.ProductVersionID = @ProductVersionID
		AND (ISNULL(@IncludeArchive,1) = 1 OR rs.Archive = @IncludeArchive)
	) a
	ORDER BY a.StartDate DESC, a.ReleaseSessionID;
