...
/****** Object:  StoredProcedure [dbo].[ReleaseDSEReport_Get]    Script Date: 5/7/2018 10:28:43 AM ******/
DROP PROCEDURE [dbo].[ReleaseDSEReport_Get]
GO
...
CREATE procedure [dbo].[ReleaseDSEReport_Get]
...
begin
	...
	declare @sqlDepSum nvarchar(max) = '';
	...
	select rst.AORReleaseID,
		...
		wit.AssignedToRankID,
		wit.COMPLETIONPERCENT,
		wrta.WTS_RESOURCE_TYPEID
	into #SubTaskResourceData
	from WORKITEM_TASK wit
	join AORReleaseSubTask rst
	on wit.WORKITEM_TASKID = rst.WORKITEMTASKID
	left join AORRelease arl
	...
	left join WTS_SYSTEM_CONTRACT wsc
	on wi.WTS_SYSTEMID = wsc.WTS_SYSTEMID
	left join WTS_RESOURCE wrta
	on (wit.ASSIGNEDRESOURCEID = wrta.WTS_RESOURCEID
	or wit.PrimaryResourceID = wrta.WTS_RESOURCEID)
	left join WTS_SYSTEM ws
	on wi.WTS_SYSTEMID = ws.WTS_SYSTEMID
	left join WTS_SYSTEM_SUITE wss
	on ws.WTS_SYSTEM_SUITEID = wss.WTS_SYSTEM_SUITEID
	where (wit.ProductVersionID = arl.ProductVersionID and wit.AssignedToRankID in (27,28,29,38))
	and (isnull(@ReleaseIDs,'') = '' OR CHARINDEX(',' + convert(nvarchar(10), isnull(arl.ProductVersionID, 0)) + ',', ',' + @ReleaseIDs + ',') > 0)
	and (isnull(@WorkloadAllocations,'') = '' OR CHARINDEX(',' + convert(nvarchar(10), isnull(arl.WorkloadAllocationID, 0)) + ',', ',' + @WorkloadAllocations + ',') > 0)
	and (isnull(@VisibleToCustomer,'') = '' OR CHARINDEX(',' + convert(nvarchar(10), isnull(arl.AORCustomerFlagship, 0)) + ',', ',' + @VisibleToCustomer + ',') > 0)
	and (isnull(@ContractIDs,'') = '' OR CHARINDEX(',' + convert(nvarchar(10), isnull(wsc.CONTRACTID, 0)) + ',', ',' + @ContractIDs + ',') > 0)
	...
	on wi.WTS_SYSTEMID = wsc.WTS_SYSTEMID
	left join WTS_RESOURCE wrta
	on (wi.ASSIGNEDRESOURCEID = wrta.WTS_RESOURCEID
	or wi.PRIMARYRESOURCEID =  wrta.WTS_RESOURCEID)
	left join WTS_SYSTEM ws
	on wi.WTS_SYSTEMID = ws.WTS_SYSTEMID
	left join WTS_SYSTEM_SUITE wss
	on ws.WTS_SYSTEM_SUITEID = wss.WTS_SYSTEM_SUITEID
	where (wi.ProductVersionID = arl.ProductVersionID and wi.AssignedToRankID in (27,28,29,38))
	and (isnull(@ReleaseIDs,'') = '' OR CHARINDEX(',' + convert(nvarchar(10), isnull(arl.ProductVersionID, 0)) + ',', ',' + @ReleaseIDs + ',') > 0)
	and (isnull(@WorkloadAllocations,'') = '' OR CHARINDEX(',' + convert(nvarchar(10), isnull(arl.WorkloadAllocationID, 0)) + ',', ',' + @WorkloadAllocations + ',') > 0)
	and (isnull(@VisibleToCustomer,'') = '' OR CHARINDEX(',' + convert(nvarchar(10), isnull(arl.AORCustomerFlagship, 0)) + ',', ',' + @VisibleToCustomer + ',') > 0)
	and (isnull(@ContractIDs,'') = '' OR CHARINDEX(',' + convert(nvarchar(10), isnull(wsc.CONTRACTID, 0)) + ',', ',' + @ContractIDs + ',') > 0)
	...
	and not exists (
		select 1
		from #SubTaskResourceData
		where AORReleaseID = rst.AORReleaseID
	)
	;

	select a.AORReleaseID,
		isnull(count(distinct case when a.WTS_RESOURCE_TYPEID = 1 then a.WTS_RESOURCEID  else null end), 0) as [1],
		isnull(count(distinct case when a.WTS_RESOURCE_TYPEID = 2 then a.WTS_RESOURCEID  else null end), 0) as [2],
		isnull(count(distinct case when a.WTS_RESOURCE_TYPEID = 3 then a.WTS_RESOURCEID  else null end), 0) as [3]
	into #ResourceMetrics
	from (
		select AORReleaseID,
			WORKITEMID as WorkTaskID,
			AssignedToRankID,
			st.WTS_RESOURCE_TYPEID,
			wrta.WTS_RESOURCEID
		from #SubTaskResourceData st
		left join WTS_RESOURCE wrta 
		on (st.ASSIGNEDRESOURCEID = wrta.WTS_RESOURCEID
		or st.PRIMARYRESOURCEID =  wrta.WTS_RESOURCEID)
		union all
		select AORReleaseID,
			WORKITEMID as WorkTaskID,
			AssignedToRankID,
			td.WTS_RESOURCE_TYPEID,
			wrta.WTS_RESOURCEID
		from #TaskResourceData td
		left join WTS_RESOURCE wrta 
		on (td.ASSIGNEDRESOURCEID = wrta.WTS_RESOURCEID
		or td.PRIMARYRESOURCEID =  wrta.WTS_RESOURCEID)
	) a
	...
	select arl.AORReleaseID, aorid,
		case when arlo.AORRelease_OverrideID is not null then arlo.PriorityID else
	...
	into #AOREstimate
	from AORRelease arl
	left join (select aear.AORReleaseID,
		...
		where exists (
			...
			where ard.AORReleaseID = aear.AORReleaseID
		)
		group by aear.AORReleaseID
	) a
	on arl.AORReleaseID = a.AORReleaseID
	join AORRelease_Override arlo
	on arl.AORRelease_OverrideID = arlo.AORRelease_OverrideID
	;
	...
	where exists (
		...
		where rs.ReleaseScheduleID = ard.DeliverableID
	)
	...
	and (isnull('40,39','') = '' OR CHARINDEX(',' + convert(nvarchar(10), isnull(arl.ProductVersionID, 0)) + ',', ',' + '40,39' + ',') > 0)		
	) 
	select a.ProductVersionID,
	a.CONTRACTID,
	a.WorkloadAllocationID,
	a.[WTS_SYSTEM_SUITEID],
	a.WorkloadPriority,
...
from (
	...
	left join #WorkloadPriority wps
			on wd.AORReleaseID = wps.AORReleaseID
			and wd.WORKITEMID = wps.WorkTaskID
...
FULL OUTER JOIN   (
	...
	(
		SELECT
			r.*,
			ROW_NUMBER() OVER(PARTITION BY r.ProductVersionID ORDER BY r.cntSuite DESC) rn
		FROM #SuiteCount r
	) r
	WHERE r.rn = 1
	;
	...
	SELECT r.ProductVersionID,
		...
		WHERE r.rn = 1
		and arl.WorkloadAllocationID = 22
		and (isnull(@SystemSuiteIDs, '') = '' or charindex(',' + convert(nvarchar(10), isnull(wsys.WTS_SYSTEM_SUITEID, 0)) + ',', ',' + @SystemSuiteIDs + ',') > 0)
	...
	;
			...
			from ProductVersion pv
				...
				left join [CONTRACT] c
				on dc.CONTRACTID = c.CONTRACTID
				--where rs.[PlannedEnd] >= convert(nvarchar(30), getdate()) 
				--and rs.[ActualEnd] is null
		...
		;
			...
			Select Distinct
				...
				convert(nvarchar(10), wfd.CONTRACTID) as CONTRACTID1,
				isnull(wfd.[CONTRACT], '') as [CONTRACT1],
				convert(nvarchar(10), wsd.CONTRACTID) as CONTRACTID2,
				isnull(wsd.[CONTRACT], '') as [CONTRACT2],
				convert(nvarchar(10), wcd.CONTRACTID) as CONTRACTID3,
				isnull(wcd.[CONTRACT], '') as [CONTRACT3],
				convert(nvarchar(10), wcd2.CONTRACTID) as CONTRACTID4,
				isnull(wcd2.[CONTRACT], '') as [CONTRACT4]
	...
	set @sqlSD = @sqlSD + '		UNION ALL
		...
		order by a.ProductVersionSort, upper(a.ProductVersion), a.SortOrder
		';
	...
	with w_FirstDeliverable as (
			select z.ProductVersionID,
				...
				isnull(convert(nvarchar(10),  isnull(sum(resm.[1]), 0)) + ''.'' +
					convert(nvarchar(10),  isnull(sum(resm.[2]), 0)) + ''.'' +
					convert(nvarchar(10),  isnull(sum(resm.[3]), 0)) +
					'' ('' + convert(nvarchar(10),  isnull(sum(resm.[1]), 0) + isnull(sum(resm.[2]), 0) + isnull(sum(resm.[3]), 0), 0) + '')'', ''0.0.0 (0)'') as ResourceMetrics,
				isnull(convert(nvarchar(10),  isnull(sum(wps.[1]), 0)) + ''.'' +
			convert(nvarchar(10),  isnull(sum(wps.[2]), 0)) + ''.'' +
			convert(nvarchar(10),  isnull(sum(wps.[3]), 0)) + ''.'' +
			convert(nvarchar(10),  isnull(sum(wps.[4]), 0)) + ''.'' +
			...
			';
				...
				if (@ContractIDs != '') set @sqlSD2 = @sqlSD2 + 'and isnull(dc.CONTRACTID, 0) in (' + @ContractIDs + ') ';
			set @sqlSD2 = @sqlSD2 + ' 
			) z
			left join AORReleaseDeliverable ard
			on z.ReleaseScheduleID = ard.DeliverableID
			...
			left join AORReleaseTask art
			on arl.AORReleaseID = art.AORReleaseID
			left join #ResourceMetrics resm
			on art.AORReleaseID = resm.AORReleaseID
			left join #WorkloadPriority wps
			on art.AORReleaseID = wps.AORReleaseID
			and art.WORKITEMID = wps.WorkTaskID
			--where z.keyOrder > 3
		...
		isnull(convert(nvarchar(10), pv.ProductVersionID),'''') as ProductVersionID,
		convert(nvarchar(10), c.CONTRACTID) as CONTRACTID,
		isnull(convert(nvarchar(100), C.[CONTRACT]),'''') as [CONTRACT],
		isnull(convert(nvarchar(10), rs.[ReleaseScheduleID]),'''') as ReleaseScheduleID,
		isnull(rs.[ReleaseScheduleDeliverable], '''') as ReleaseScheduleDeliverable,
		isnull(rs.[Description], '''') as ReleaseScheduleDescr,
		...
		isnull(wfd.ResourceMetrics,'''') as ResourceMetrics,
		isnull(wfd.WorkloadPriority,'''') as WorkloadPriority,
		 case when rs.[ActualEnd] is null and rs.[PlannedEnd] <= convert(nvarchar(30), getdate()) then ''1'' 
					   when rs.[ActualEnd] <= convert(nvarchar(30), getdate()) then ''1'' else ''0'' end as IsDeployed,
		rs.[SORT_ORDER],
		...
		on dc.CONTRACTID = c.CONTRACTID
		 join w_FirstDeliverable wfd
			...
			order by case when rs.[ActualEnd] is null and rs.[PlannedEnd] <= convert(nvarchar(30), getdate()) then ''1'' 
					   when rs.[ActualEnd] <= convert(nvarchar(30), getdate()) then ''1'' else ''0'' end, rs.[SORT_ORDER],
			 case when AORCount > 0 then 1 else 2 end, (case when rs.[ActualEnd] is null and rs.[PlannedEnd] >= convert(nvarchar(30), getdate()) then 1 
							when rs.[ActualEnd] <= convert(nvarchar(30), getdate()) then 2 else 3 end),
					 convert(date, rs.[ActualEnd], 101)  desc, convert(date, rs.[PlannedEnd], 101)  asc
			;
			';
	--CR
	set @sqlCR = '
	...
	set @sqlCR = @sqlCR + '
		...
		from (
			select isnull(ps.[WorkloadAllocation], ''-'') as WorkloadAllocation,
				...
				isnull(convert(nvarchar(10), arl.ProductVersionID),''0'') as ProductVersionID,
				isnull(pv.ProductVersion, ''-'') as ProductVersion,
				isnull(pv.SORT_ORDER, 9999) as ProductVersionSort,
				isnull(pv.Description, '''') as MissionTitle,
				isnull(pv.Narrative, '''') as ReleaseNarrative,
	...
	set @sqlCR = @sqlCR + '
			group by isnull(ps.[WorkloadAllocation], ''-''),
				...
				isnull(convert(nvarchar(10), arl.ProductVersionID),''0''),
				isnull(pv.ProductVersion, ''-''),
				isnull(pv.SORT_ORDER, 9999),
	...
	set @sqlAOR = '
		with w_primary_system as (
			select distinct ars.AORReleaseID,
				wsy.WTS_SYSTEM,
				wsys.WTS_SYSTEM_SUITE,
				...
				stuff((
					select 
						'','' + convert(nvarchar(10), con.[ContractID]) 
					from [CONTRACT] con
						 join WTS_SYSTEM_CONTRACT wsc1
						on con.CONTRACTID = wsc1.CONTRACTID
						and wsc.WTS_SYSTEMID = wsc1.WTS_SYSTEMID
						order by isnull(con.SORT_ORDER, 9999)
					for xml path(''''), type).value(''.'', ''nvarchar(max)''), 1, 1, '''') as ContractID
				,stuff((
					select 
						'','' + isnull(con.[CONTRACT], ''-'') 
					from [CONTRACT] con
						...
						and wsc.WTS_SYSTEMID = wsc1.WTS_SYSTEMID
						order by isnull(con.SORT_ORDER, 9999)
					for xml path(''''), type).value(''.'', ''nvarchar(max)''), 1, 1, '''') as [CONTRACT]
				,stuff((
					select 
						'','' +convert(nvarchar(10), isnull(con.SORT_ORDER, 9999)) 
					from [CONTRACT] con
						 join WTS_SYSTEM_CONTRACT wsc1
						on con.CONTRACTID = wsc1.CONTRACTID
						and wsc.WTS_SYSTEMID = wsc1.WTS_SYSTEMID
						order by isnull(con.SORT_ORDER, 9999)
					for xml path(''''), type).value(''.'', ''nvarchar(max)''), 1, 1, '''') as ContractSort
			...
			on wsy.[WTS_SYSTEM_SUITEID] = wsys.[WTS_SYSTEM_SUITEID]
			left join WTS_SYSTEM_CONTRACT wsc
			on ars.WTS_SYSTEMID = wsc.WTS_SYSTEMID
			left join [CONTRACT] c
			on wsc.CONTRACTID = c.CONTRACTID
			where ars.[Primary] = 1
		),
		...
		,w_AttachmentNames as (
		 select distinct 
			 aor1.AORID as AOR_ID,
					arl1.AORName as [AOR Name],
					arl1.AORReleaseID as AORRelease_ID,
			stuff((
			select 
				'', '' + art.AORReleaseAttachmentName 
				from AOR
				join AORRelease arl
				on AOR.AORID = arl.AORID
				and AOR.AORID = aor1.AORID
				and arl.AORReleaseID = arl1.AORReleaseID
				join AORReleaseAttachment art
				on arl.AORReleaseID = art.AORReleaseID
				order by upper(art.AORReleaseAttachmentName)
			for xml path(''''), type).value(''.'', ''nvarchar(max)''), 1, 0, '''') as AORReleaseAttachmentName
			from AOR aor1
			join AORRelease arl1
			on aor1.AORID = arl1.AORID
			join AORReleaseAttachment art
			on arl1.AORReleaseID = art.AORReleaseID
			)
			,w_CR_UpdatedDate as (
	...
	set @sqlAOR = @sqlAOR + '
		select * from (
			select convert(nvarchar(10), acr.CRID) as CRID,
				...
				isnull(convert(nvarchar(10),  isnull(resm.[1], 0)) + ''.'' +
					convert(nvarchar(10),  isnull(resm.[2], 0)) + ''.'' +
					convert(nvarchar(10),  isnull(resm.[3], 0)) +
					'' ('' + convert(nvarchar(10),  isnull(resm.[1], 0) + isnull(resm.[2], 0) + isnull(resm.[3], 0), 0) + '')'', ''0.0.0 (0)'') as ResourceMetrics,
				isnull(convert(nvarchar(10),  isnull(sum(wps.[1]), 0)) + ''.'' +
					convert(nvarchar(10),  isnull(sum(wps.[2]), 0)) + ''.'' +
					convert(nvarchar(10),  isnull(sum(wps.[3]), 0)) + ''.'' +
					convert(nvarchar(10),  isnull(sum(wps.[4]), 0)) + ''.'' +
			...
			set @sqlAOR = @sqlAOR + 'convert(nvarchar(10), arl.WorkloadAllocationID) as WorkloadAllocationID,
				...
				convert(nvarchar(10), psy.CONTRACTID) as CONTRACTID,
				isnull(psy.[CONTRACT], ''-'') as [CONTRACT],
				isnull(psy.ContractSort, 9999) as ContractSort,
				...
				isnull(mpsl.MaxStatusType,'''') as SuitePD2TDRType,
				isnull(mpsl.MaxStatus,'''') as SuitePD2TDR,
				isnull(mpsl.WorkloadPriority,''0.0.0.0.0.0 (0, 0%)'') as SuiteWP,
				...
				isnull(p.[PRIORITY],''Low (Automated)'') as AORRisk,
				isnull(p.SORT_ORDER,''1'') as AORRiskSort
				';
		...
		set @sqlAOR = @sqlAOR + '
			...
			left join #MAX_PD2TDR_SUITE_LVL mpsl
			on arl.ProductVersionID = mpsl.ProductVersionID 
			and wsc.CONTRACTID = mpsl.CONTRACTID  
			and arl.WorkloadAllocationID = mpsl.WorkloadAllocationID 
			and psy.[WTS_SYSTEM_SUITEID] = mpsl.[WTS_SYSTEM_SUITEID] 
			...
			on p.PRIORITYID = ae.PriorityID
			where isnull(wsc.[Primary], 1) = 1
			and isnull(AOR.Archive, 0) = 0
			and isnull(arl.WorkloadAllocationID, 0) != 22
			';
	...
	set @sqlAOR = @sqlAOR + '
			group by convert(nvarchar(10), acr.CRID),
				...
				isnull(convert(nvarchar(10),  isnull(resm.[1], 0)) + ''.'' +
					convert(nvarchar(10),  isnull(resm.[2], 0)) + ''.'' +
					convert(nvarchar(10),  isnull(resm.[3], 0)) +
					'' ('' + convert(nvarchar(10),  isnull(resm.[1], 0) + isnull(resm.[2], 0) + isnull(resm.[3], 0), 0) + '')'', ''0.0.0 (0)''),
				...
				convert(nvarchar(10), psy.CONTRACTID),
				isnull(psy.[CONTRACT], ''-''),
				isnull(psy.ContractSort, 9999),
				...
				isnull(mpsl.MaxStatusType,''''),
				isnull(mpsl.MaxStatus,''''),
				isnull(mpsl.WorkloadPriority,''0.0.0.0.0.0 (0, 0%)''),
				...
				isnull(p.[PRIORITY],''Low (Automated)''),
				isnull(p.SORT_ORDER,''1'') 
	...
	set @sqlDepSum = '
		...
		,w_AttachmentNames as (
		 select distinct 
			...
			,w_ScheduleDate as (
				select ard.AORReleaseID
				,rs.ReleaseScheduleID
				,rs.[SORT_ORDER]
				,rs.ReleaseScheduleDeliverable
				, rs.[Description] as Deployment
				...
				,ard.[Weight]
				, isnull(convert(nvarchar(10),rs.[PlannedEnd]), ''9999'') as ScheduledDate
				, case when rs.[ActualEnd] is null and rs.[PlannedEnd] <= convert(nvarchar(30), getdate()) then 1 
					   when rs.[ActualEnd] <= convert(nvarchar(30), getdate()) then 1 else 0 end as IsDeployed
				from 
				AORReleaseDeliverable ard
				...
				--group by ard.AORReleaseID, rs.ReleaseScheduleID,rs.[SORT_ORDER],rs.ReleaseScheduleDeliverable,rs.[Description],ard.[Weight]
			)
			--,w_DeployAORs as (
			--	 select distinct 
			--	 rs1.ProductVersionID,
			--	 rs1.ReleaseScheduleDeliverable,
			--	 ard1.AORReleaseID,
			--		 --aor1.AORID as AOR_ID,
			--			--	arl1.AORName as [AOR Name],
			--			--	arl1.AORReleaseID as AORRelease_ID,
			--		stuff((
			--		select 
			--			'','' + convert(nvarchar(10), aor.AORID) 
			--		from AORReleaseDeliverable ard
			--			 join ReleaseSchedule rs
			--			on ard.DeliverableID = rs.ReleaseScheduleID
			--			and ard.DeliverableID = rs1.ReleaseScheduleID
			--			and rs.ProductVersionID = rs1.ProductVersionID
			--			join AORRelease arl
			--			on ard.AORReleaseID = arl.AORReleaseID
			--			join AOR aor
			--			on arl.AORID = aor.AORID
			--			where AOR.Archive = 0
			--			order by upper(aor.AORID)
			--		for xml path(''''), type).value(''.'', ''nvarchar(max)''), 1, 1, '''') as AORs,
			--		stuff((
			--		select 
			--			'','' + convert(nvarchar(10), dc.[ContractID]) 
			--		from ReleaseSchedule rs
			--			join [DeploymentContract] dc
			--			on rs.ReleaseScheduleID = dc.[DeliverableID]
			--			and rs1.ReleaseScheduleID = dc.[DeliverableID]
			--			order by upper(dc.[ContractID])
			--		for xml path(''''), type).value(''.'', ''nvarchar(max)''), 1, 1, '''') as Contracts
			--		from AORReleaseDeliverable ard1
			--		 join ReleaseSchedule rs1
			--		on ard1.DeliverableID = rs1.ReleaseScheduleID
			--		join AORRelease arl1
			--		on ard1.AORReleaseID = arl1.AORReleaseID
			--		join AOR aor1
			--		on arl1.AORID = aor1.AORID
			--		where AOR1.Archive = 0
			--		)
		';
	...
	set @sqlDepSum = @sqlDepSum + '
		select * from (
			select convert(nvarchar(10), acr.CRID) as CRID,
				...
				isnull(psy.WTS_SYSTEM, '''') as PrimaryWebsystem,
				isnull(psy.WTS_SYSTEM_SUITE, '''') as PriWebsystemSuite,
				isnull(psy.SYSTEM_SUITE_ABBREV, '''') as PriWebsystemSuiteAbbrev,
				isnull(psy.WTS_SYSTEM_SUITE_SORT, 9999) as WTS_SYSTEM_SUITE_SORT,
				isnull(invs.[STATUS], ''-'') as InvestigationStatus,
				isnull(ts.[STATUS], ''-'') as TechnicalStatus,
				isnull(cds.[STATUS], ''-'') as CustomerDesignStatus,
			...
			set @sqlDepSum = @sqlDepSum + 'convert(nvarchar(10), arl.WorkloadAllocationID) as WorkloadAllocationID,
				isnull(ps.[WorkloadAllocation], ''-'') as [WorkloadAllocation],
				isnull(ps.Sort, 9999) as WorkloadAllocationSort,
				isnull(awt.AORWorkTypeName, '''') as AORWorkTypeName,
				convert(nvarchar(10), awt.AORWorkTypeID) as AORWorkTypeID,
				...
				convert(nvarchar(10), isnull(sum(wps.[1]), 0) + isnull(sum(wps.[2]), 0) + isnull(sum(wps.[3]), 0) + isnull(sum(wps.[4]), 0) + isnull(sum(wps.[5+]), 0) + isnull(sum(wps.[6]), 0)) as TaskCount,
				isnull(convert(nvarchar(10),rssd.ScheduledDate), ''9999'') as ScheduledDate,
				convert(nvarchar(10), rssd.ReleaseScheduleID) as ReleaseScheduleID,
				isnull(rssd.ReleaseScheduleDeliverable,'''') as ReleaseScheduleDeliverable,
				isnull(rssd.Deployment,''-'') as Deployment,
				isnull(rssd.[SORT_ORDER], 9999) as DeploymentSort,
				isnull(convert(nvarchar(10), rssd.[Weight]),'''') as [Weight],
				--isnull(depa.AORs,'''') as AORs,
				--isnull(depa.Contracts,'''') as Contracts,
				crs.STATUS  as CyberReview,
				isnull(cmmis.[STATUS],'''') as CMMI,
				isnull(mpa.MaxStatusType,'''') as PD2TDRType,
				isnull(mpa.MaxStatus,'''') as PD2TDR,
				...
				isnull(p.[PRIORITY],''Low (Automated)'') as AORRisk,
				isnull(p.SORT_ORDER,''1'') as AORRiskSort,
				isnull(rssd.risk,'''') as DeploymentRisk,
				convert(nvarchar(10),isnull(rssd.IsDeployed,''0'')) as IsDeployed
				';
		...
		set @sqlDepSum = @sqlDepSum + '
			...
			left join w_Task_UpdatedDate wtu
			on arl.AORReleaseID = wtu.AORReleaseID
			--left join w_DeployAORs depa
			--on arl.AORReleaseID = depa.AORReleaseID
			--and arl.ProductVersionID = depa.ProductVersionID
			left join [WorkloadAllocation] ps
			on arl.WorkloadAllocationID = ps.WorkloadAllocationID
			left join #AOREstimate ae
			on arl.AORReleaseID = ae.AORReleaseID
			...
			if (@ReleaseIDs != '') set @sqlDepSum = @sqlDepSum + 'and isnull(arl.ProductVersionID, 0) in (' + @ReleaseIDs + ') ';
			if (@AORTypes != '') set @sqlDepSum = @sqlDepSum + 'and isnull(awt.AORWorkTypeID, 0) in (' + @AORTypes + ') ';
			if (@ContractIDs != '') set @sqlDepSum = @sqlDepSum + 'and isnull(wsc.CONTRACTID, 0) in (' + @ContractIDs + ') ';
			if (@SystemSuiteIDs != '') set @sqlDepSum = @sqlDepSum + 'and isnull(psy.WTS_SYSTEM_SUITEID, 0) in (' + @SystemSuiteIDs + ') ';
			if (@WorkloadAllocations != '') set @sqlDepSum = @sqlDepSum + 'and isnull(arl.WorkloadAllocationID, 0) in (' + @WorkloadAllocations + ') ';
			if (@VisibleToCustomer != '') set @sqlDepSum = @sqlDepSum + 'and isnull(arl.AORCustomerFlagship, 0) in (' + @VisibleToCustomer + ') ';
			--if (@ScheduledDeliverables != '') set @sqlAOR = @sqlAOR + 'and isnull(rs.ReleaseScheduleID, 0) in (' + @ScheduledDeliverables + ') ';
			--if (@VisibleToCustomer != '') set @sqlAOR = @sqlAOR + 'and isnull(rs.Visible, 0) in (' + @VisibleToCustomer + ') ';
	...
	set @sqlDepSum = @sqlDepSum + '
			group by convert(nvarchar(10), acr.CRID),
				...
				isnull(psy.WTS_SYSTEM, ''''),
				isnull(psy.WTS_SYSTEM_SUITE, ''''),
				isnull(psy.SYSTEM_SUITE_ABBREV, ''''),
				isnull(psy.WTS_SYSTEM_SUITE_SORT, 9999),
				isnull(invs.[STATUS], ''-''),
				isnull(ts.[STATUS], ''-''),
				isnull(cds.[STATUS], ''-''),
				...
				isnull(AOR.AORID, 0),
				isnull(convert(nvarchar(10),rssd.ScheduledDate), ''9999''),
				convert(nvarchar(10), rssd.ReleaseScheduleID),
				isnull(rssd.ReleaseScheduleDeliverable,''''),
				isnull(rssd.Deployment,''-''),
				isnull(rssd.[SORT_ORDER], 9999),
				isnull(convert(nvarchar(10), rssd.[Weight]),''''),
				--isnull(depa.AORs,''''),
				--isnull(depa.Contracts,''''),
				crs.STATUS,
				isnull(cmmis.[STATUS],''''),
				isnull(mpa.MaxStatusType,''''),
				isnull(mpa.MaxStatus,''''),
				...
				isnull(p.SORT_ORDER,''1''),
				isnull(rssd.risk,''''),
				convert(nvarchar(10),isnull(rssd.IsDeployed,''0''))
		) a
		order by ProductVersionSort, WorkloadAllocationSort,  ContractSort, CONTRACT, IsDeployed, AORRiskSort desc, AORRisk, ScheduledDate, DeploymentSort, AORID;
		--WTS_SYSTEM_SUITE_SORT,PriWebsystemSuite,
	';
	...
	if @Debug = 1
		begin
			select @sqlParameters + ' ' + @sqlImages + ' ' + @sqlNarrative + ' ' + @sqlPD2TDR + ' ' + @sqlDepLvl + ' ' + @sqlSD + ' ' + @sqlCR + ' ' + @sqlAOR + ' ' + @sqlDepSum + ' ' + @sqlSR ;
		end;
	else
		begin
			...
			execute sp_executesql @sqlDepSum;
	...
	if object_id('tempdb..#MAX_PD2TDR_SUITE_LVL') is not null
		begin
			drop table #MAX_PD2TDR_SUITE_LVL;
		end;
	if object_id('tempdb..#TaskResourceData') is not null
		begin
			drop table #TaskResourceData;
		end;
	if object_id('tempdb..#SubTaskResourceData') is not null
		begin
			drop table #SubTaskResourceData;
		end;
	if object_id('tempdb..#ResourceMetrics') is not null
		begin
			drop table #ResourceMetrics;
		end;
	if object_id('tempdb..#DeploymentEstimate') is not null
		begin
			drop table #DeploymentEstimate;
		end;
	if object_id('tempdb..#AOREstimate') is not null
		begin
...
SELECT 'Executing File [Procedures\ReleaseDSEReport_Get.sql]';
GO
