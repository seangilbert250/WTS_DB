...
/****** Object:  StoredProcedure [dbo].[ReleaseSessionBreakoutList_Get]    Script Date: 6/6/2018 9:53:50 AM ******/
DROP PROCEDURE [dbo].[ReleaseSessionBreakoutList_Get]
GO
...
CREATE PROCEDURE [dbo].[ReleaseSessionBreakoutList_Get]
...
BEGIN
	...
	select wit.WORKITEM_TASKID, wit.WORKITEMID, wit.TASK_NUMBER, rs.ReleaseSessionID, wsy.WTS_SYSTEM_SUITEID, wsy.WTS_SYSTEMID, wsc.CONTRACTID, wma.AORReleaseID, rdma.AORReleaseID as RelDepAORReleaseID
	into #ClosedSubTask
	...
	on wit.WORKITEM_TASKID = rdma.WORKITEMTASKID
	...
	select wi.WORKITEMID, wi.CREATEDDATE, wih.CREATEDDATE as ClosedDate, rs.ReleaseSessionID, wsy.WTS_SYSTEM_SUITEID, wsy.WTS_SYSTEMID, wsc.CONTRACTID, wma.AORReleaseID, rdma.AORReleaseID as RelDepAORReleaseID
	into #ClosedTask
	from WORKITEM wi
	...
	on wi.WORKITEMID = rdma.WORKITEMID
	...
	from (
		...
		from (
			(select wit.WORKITEM_TASKID, wit.WORKITEMID, wit.TASK_NUMBER, wit.CREATEDDATE as SessionDate, rs.ReleaseSessionID, 'Open' as SessionStatus, wsy.WTS_SYSTEM_SUITEID, wsy.WTS_SYSTEMID, wsc.CONTRACTID, wma.AORReleaseID, rdma.AORReleaseID as RelDepAORReleaseID
			from WORKITEM_TASK wit
			join ReleaseSession rs
			...
			union all
			select wit.WORKITEM_TASKID, wit.WORKITEMID, wit.TASK_NUMBER, wth.CREATEDDATE as SessionDate, rs.ReleaseSessionID, case when wth.NewValue = 'Closed' then 'Closed' else 'Open' end as SessionStatus, wsy.WTS_SYSTEM_SUITEID, wsy.WTS_SYSTEMID, wsc.CONTRACTID, wma.AORReleaseID, rdma.AORReleaseID as RelDepAORReleaseID
			from WORKITEM_TASK wit
			join WorkItem_Task_History wth
			on wit.WORKITEM_TASKID = wth.WORKITEM_TASKID
			join ReleaseSession rs
			...
			union all
			select wit.WORKITEM_TASKID, wit.WORKITEMID, wit.TASK_NUMBER, wit.CREATEDDATE as SessionDate, rs.ReleaseSessionID, 'Closed' as SessionStatus, wsy.WTS_SYSTEM_SUITEID, wsy.WTS_SYSTEMID, wsc.CONTRACTID, wma.AORReleaseID, rdma.AORReleaseID as RelDepAORReleaseID
			from WORKITEM_TASK wit
			join ReleaseSession rs
			on wit.ProductVersionID = rs.ProductVersionID and convert(date, wit.CREATEDDATE) between convert(date, rs.StartDate) and (dateadd(day, rs.Duration, convert(date, rs.StartDate)))
			join WORKITEM wi
	...
	union all
	select wit.WORKITEM_TASKID, wit.WORKITEMID, wit.TASK_NUMBER, rs.StartDate as SessionDate, rs.ReleaseSessionID, 'Open' as SessionStatus, wsy.WTS_SYSTEM_SUITEID, wsy.WTS_SYSTEMID, wsc.CONTRACTID, wma.AORReleaseID, rdma.AORReleaseID as RelDepAORReleaseID, 0 as rn
	from WORKITEM_TASK wit
	join ReleaseSession rs
	...
	from (
		...
		from (
			(select wi.WORKITEMID, wi.CREATEDDATE as SessionDate, rs.ReleaseSessionID, 'Open' as SessionStatus, wsy.WTS_SYSTEM_SUITEID, wsy.WTS_SYSTEMID, wsc.CONTRACTID, wma.AORReleaseID, rdma.AORReleaseID as RelDepAORReleaseID
			from WORKITEM wi
			join ReleaseSession rs
			...
			union all
			select wi.WORKITEMID, wih.CREATEDDATE as SessionDate, rs.ReleaseSessionID, case when wih.NewValue = 'Closed' then 'Closed' else 'Open' end as SessionStatus, wsy.WTS_SYSTEM_SUITEID, wsy.WTS_SYSTEMID, wsc.CONTRACTID, wma.AORReleaseID, rdma.AORReleaseID as RelDepAORReleaseID
			from WORKITEM wi
			join WorkItem_History wih
			on wi.WORKITEMID = wih.WORKITEMID
			join ReleaseSession rs
			...
			union all
			select wi.WORKITEMID, wi.CREATEDDATE as SessionDate, rs.ReleaseSessionID, 'Closed' as SessionStatus, wsy.WTS_SYSTEM_SUITEID, wsy.WTS_SYSTEMID, wsc.CONTRACTID, wma.AORReleaseID, rdma.AORReleaseID as RelDepAORReleaseID
			from WORKITEM wi
			join ReleaseSession rs
			on wi.ProductVersionID = rs.ProductVersionID and convert(date, wi.CREATEDDATE) between convert(date, rs.StartDate) and (dateadd(day, rs.Duration, convert(date, rs.StartDate)))
			join WTS_SYSTEM wsy
	...
	from #LastStatusTask
	union all
	select wi.WORKITEMID, rs.StartDate as SessionDate, rs.ReleaseSessionID, 'Open' as SessionStatus, wsy.WTS_SYSTEM_SUITEID, wsy.WTS_SYSTEMID, wsc.CONTRACTID, wma.AORReleaseID, rdma.AORReleaseID as RelDepAORReleaseID, 0 as rn
	from WORKITEM wi
	join ReleaseSession rs
	...
	select wi.WORKITEMID, rs.ReleaseSessionID, wsy.WTS_SYSTEM_SUITEID, wsy.WTS_SYSTEMID, wsc.CONTRACTID, wma.AORReleaseID, rdma.AORReleaseID as RelDepAORReleaseID
	into #CreatedTask
	from WORKITEM wi
	join ReleaseSession rs
	on wi.ProductVersionID = rs.ProductVersionID and convert(date, wi.CREATEDDATE) between convert(date, rs.StartDate) and (dateadd(day, rs.Duration, convert(date, rs.StartDate)))
	...
	select a.[Resource], a.Dev, a.Biz, a.ReleaseSessionID, a.WTS_SYSTEM_SUITEID, a.WTS_SYSTEMID, a.CONTRACTID, a.AORReleaseID, a.RelDepAORReleaseID
	into #ResourceSubTask
	from (
		select wth.NewValue as [Resource], case when res.WTS_RESOURCE_TYPEID = 2 then 1 else 0 end as Dev, case when res.WTS_RESOURCE_TYPEID = 1 then 1 else 0 end as Biz, rs.ReleaseSessionID, wsy.WTS_SYSTEM_SUITEID, wsy.WTS_SYSTEMID, wsc.CONTRACTID, wma.AORReleaseID, rdma.AORReleaseID as RelDepAORReleaseID
		from WORKITEM_TASK wit
		join WorkItem_Task_History wth
		on wit.WORKITEM_TASKID = wth.WORKITEM_TASKID
		join ReleaseSession rs
		...
		union all
		select res.FIRST_NAME + ' ' + res.LAST_NAME as [Resource], case when res.WTS_RESOURCE_TYPEID = 2 then 1 else 0 end as Dev, case when res.WTS_RESOURCE_TYPEID = 1 then 1 else 0 end as Biz, rs.ReleaseSessionID, wsy.WTS_SYSTEM_SUITEID, wsy.WTS_SYSTEMID, wsc.CONTRACTID, wma.AORReleaseID, rdma.AORReleaseID as RelDepAORReleaseID
		from WORKITEM_TASK wit
		join ReleaseSession rs
		on wit.ProductVersionID = rs.ProductVersionID
		left join WTS_RESOURCE res
	...
	into #ResourceTask
	from (
		select wih.NewValue as [Resource], case when res.WTS_RESOURCE_TYPEID = 2 then 1 else 0 end as Dev, case when res.WTS_RESOURCE_TYPEID = 1 then 1 else 0 end as Biz, rs.ReleaseSessionID, wsy.WTS_SYSTEM_SUITEID, wsy.WTS_SYSTEMID, wsc.CONTRACTID, wma.AORReleaseID, rdma.AORReleaseID as RelDepAORReleaseID
		from WORKITEM wi
		join WorkItem_History wih
		on wi.WORKITEMID = wih.WORKITEMID
		join ReleaseSession rs
	...
	select a.*,
		...
		isnull(round((cast(a.[Total Closed] as float) / nullif(cast(a.[Total Closed] + a.[Total Open] as float), 0)) * 100, 0), 0) as [Percent Closed],
		a.[Dev (Resources)] + a.[Biz (Resources)] as [Total Resources]
	into #DeploymentMetrics
	from (
		select dep.ReleaseScheduleID,
			dep.ReleaseScheduleDeliverable + ' ' + dep.[Description] as ReleaseScheduleDeliverable,
			rs.ReleaseSessionID,
			pv.ProductVersionID,
			pv.ProductVersion,
			convert(date, rs.StartDate) as StartDate,
			...
			where ReleaseSessionID = rs.ReleaseSessionID
			and SessionStatus = 'Closed'
			and exists ( --Deployment AOR
				select 1
				from AORReleaseDeliverable ard
				where ard.DeliverableID = dep.ReleaseScheduleID
				and ard.AORReleaseID = RelDepAORReleaseID
			)
			and exists ( --Deployment Contract
				select 1
				from DeploymentContract dc
				where dc.DeliverableID = dep.ReleaseScheduleID
				and dc.ContractID = CONTRACTID
			)) +
			(select count(distinct WORKITEMID)
			from #LastStatusTaskFinal
			where ReleaseSessionID = rs.ReleaseSessionID
			...
			and exists ( --Deployment AOR
				...
				from AORReleaseDeliverable ard
				where ard.DeliverableID = dep.ReleaseScheduleID
				and ard.AORReleaseID = lsst.RelDepAORReleaseID
			)
			and exists ( --Deployment Contract
				select 1
				from DeploymentContract dc
				where dc.DeliverableID = dep.ReleaseScheduleID
				and dc.ContractID = lsst.CONTRACTID
			)) +
			(select count(distinct lst.WORKITEMID)
			from #LastStatusTaskFinal lst
			join ReleaseSession rs2
			...
			and exists ( --Deployment AOR
				...
				from AORReleaseDeliverable ard
				where ard.DeliverableID = dep.ReleaseScheduleID
				and ard.AORReleaseID = lst.RelDepAORReleaseID
			)
			and exists ( --Deployment Contract
				select 1
				from DeploymentContract dc
				where dc.DeliverableID = dep.ReleaseScheduleID
				and dc.ContractID = lst.CONTRACTID
			)) as [Carry-In (Closed)],
		...
		join ProductVersion pv
		on rs.ProductVersionID = pv.ProductVersionID
		cross join ReleaseSchedule dep
		where rs.ProductVersionID = @ProductVersionID
		and dep.ProductVersionID = @ProductVersionID
		and dep.PlannedEnd > dateadd(day, rs.Duration, convert(date, rs.StartDate))
	) a;
	...
	SELECT 'Deployment' as X
		,m.ReleaseScheduleDeliverable as Y
		,rs.ReleaseSessionID
		,rs.ReleaseSession
		,rs.StartDate
		,'' as EndDate
	...
	SELECT 'Contract' as X
		...
		,0 as AltSort
	FROM ReleaseSession rs
	left join #ContractMetrics m
	on rs.ReleaseSessionID = m.ReleaseSessionID
	WHERE rs.ProductVersionID = @ProductVersionID
	and rs.ReleaseSessionID = @ReleaseSessionID
	AND (ISNULL(@IncludeArchive,1) = 1 OR rs.Archive = @IncludeArchive)
	and m.[Total Tasks] > 0
	order by BreakoutSort, AltSort, Y;
