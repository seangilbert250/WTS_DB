...
/****** Object:  StoredProcedure [dbo].[Narrative_ProductVersionList_Get]    Script Date: 6/26/2018 12:49:57 PM ******/
...
BEGIN
	SELECT * FROM (
		...
		SELECT DISTINCT
			...
			,pv.ProductVersion
			,pv.Description
			,(SELECT COUNT(DISTINCT nc.CONTRACTID)
					FROM Narrative n 
					...
					AND (ISNULL(@IncludeArchive,1) = 1 OR n.Archive = 0)) AS Contract_Count
